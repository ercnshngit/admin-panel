...
export function DesignerContextProvider({ children }: { children: ReactNode }) {
  ...
  const updateComponentProp = ({
    ...
    const changedElement = elements.find(
      (e) => e.component.tag.name === componentTagName
    );

    if (!changedElement) return;

    updateElement(changedElement.code, {
      ...changedElement,
      props: changedElement.props.map((p) => {
...
export function useDesigner() {
  ...
  if (!context) {
    return {
      ...
      dragdrop: null,
      setDragdrop: null,
    } as unknown as DesignerContextType & { contextActive: false };
  }
