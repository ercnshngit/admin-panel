...
export const getEducations = async () => {
  const { data } = await axiosClient.get("/educations");
  return data;
};
...
export const getBlockComponentsBySlugAndType = async (
  slug: string,
  type_id: number
) => {
  const { data } = await axiosClient.get(
    `/block/component/get/byslug/${slug}/${type_id}`
  );
...
export const getGeneralByGroup = async (group: string) => {
  const { data: responseData } = await axiosClient.get(
    "/general/get/bygroup/" + group
  );
...
export const createDataLanguage = async (
