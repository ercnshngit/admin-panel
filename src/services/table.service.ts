...
// TODO: abi saltTable array var ya mysql schemadan cektigimiz oradan gelen type degisti ilk kurulumda patlayabilir
...
export class TableService extends BaseService {
  ...
  async updateTableWithId(tablename: string, id: number, data: any) {
    try {
      ...
      const tableColumnsTypes = await prisma.database_table_column.findMany({
        select: {
          id: true,
          name: true,
          type: true,
      ...
      const response = await (
        ...
        data: data.reduce((acc: any, item: any) => {
          const type = tableColumnsTypes.find(
            (column) => column.name == item.key
          )?.type;
          const value =
            type === "number" ||
            (type === "relation" && !isNaN(Number(item.value)))
              ? Number(item.value)
              : type === "boolean"
  ...
  async createTable(table_name: string, data: any) {
    try {
      ...
      console.log({ tableColumnsTypes });

      const tables = Prisma.ModelName;
      console.log(table_name);
      if (!(table_name in tables)) {
        throw new Error(
          `Model "${table_name}" does not exist in Prisma schema.`
        );
      }
      const response = await (
    ...
    try {
      ...
      const result = await prisma.database_table.update({
        ...
        data: {
          ...
          order: Number(tableData.order),
          group: tableData.group,
          display_column_id: Number(tableData.display_column_id),
          columns: {
                  ...
                  connect: {
                    id: inputTypesArray.filter(
                      (input_type) =>
                        input_type.name ==
                          InputTypes[
                            column.input_type_id as keyof typeof InputTypes
                          ] &&
  ...
  async migrateTableConfig(table_name: any) {
    try {
      ...
      const tableSaltColumns = tableDataArray[0].columns as {
        name: string;
        type: string;
        input_type_id: number;
      }[];

      console.log(tableSaltColumns);

      const tableConfigColumns = result2.columns;
      // eksik columnlar bulunuyor.
      tableSaltColumns.forEach((element) => {
      ...
      const result3 = await prisma.database_table.update({
        ...
        data: {
          columns: {
            create: missingColumns.map((column, index: number) => ({
              name: column.name,
              order: index,
              is_hidden: column.name === "id" ? true : false,
              type: column.type,
              input_type: {
                connect: {
                  id:
                    inputTypesArray.filter((input_type) => {
                      return (
                        input_type.name ==
                          InputTypes[
                            column.input_type_id as keyof typeof InputTypes
