...
export class MenuService extends BaseService {
  ...
  async changeOrder(data: { from: number; to: number; subMenu: boolean }) {
    try {
      ...
      let toMenu = await prisma.menu.findFirst({ where: { id: data.to } });
      ...
      // submenu true ise alt menuleri de degistir
      if (data.subMenu) {
        if (data.from === data.to && fromMenu.previous_id) {
          toMenu = await prisma.menu.findFirst({
        ...
        const fromMenuPrev = await prisma.menu.findFirst({
          where: { next_id: fromMenu.id },
        });

        if (fromMenuPrev) {
          const updatedFromMenuPrev = await prisma.menu.update({
            where: { id: fromMenuPrev.id },
            data: {
              next_id: fromMenu.next_id,
            },
          });
        }
        if (fromMenuNext) {
          const updatedFromMenuNext = await prisma.menu.update({
            ...
            data: {
              previous_id: fromMenu.previous_id,
            },
          });
        }

        const lastItemBelongsToToMenu = await prisma.menu.findFirst({
          where: { menu_belong_id: toMenu.id, next_id: null },
        });
        let updatedFromMenu;

        if (!lastItemBelongsToToMenu) {
          console.log("buraya");
          updatedFromMenu = await prisma.menu.update({
            where: { id: data.from },
            data: {
              menu_belong_id: toMenu.id,
              previous_id: null,
              next_id: null,
            },
          });
          ...
          updatedFromMenu = await prisma.menu.update({
            ...
            data: {
              menu_belong_id: toMenu.id,
              previous_id: lastItemBelongsToToMenu.id,
              next_id: null,
            },
          });
          const updatedLastItemBelongsToToMenu = await prisma.menu.update({
            where: { id: lastItemBelongsToToMenu.id },
            data: {
              next_id: fromMenu.id,
            },
          });
      ...
      //ayni itemse ve belong id varsa
      if (data.from === data.to && fromMenu.menu_belong_id) {
        toMenu = await prisma.menu.findFirst({
          where: { id: fromMenu.menu_belong_id },
        });
